...
use crate::field::{FieldOverlay, FieldPreview, FieldSelection, SelectionStatus};
use catan_core::gameplay::game::projection::{
    GameProjection, LegalDecisionOptions, PrivatePlayerProjection, TradeOfferProjection,
    TradeSessionProjection, game_projection_summary,
};
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum TradeResponseMenuAction {
    Accept(TradeOfferId),
    Reject,
    Counter(PlayerTrade),
}
...
impl CardGlyph {
    ...
    #[allow(dead_code)]
    pub fn face_style(mut self, style: Style) -> Self {
        self.style.face = style;
        self
    }

    pub fn index_top(mut self, value: u16, style: Style) -> Self {
        self.indices[0] = Some(CardGlyphIndex { value, style });
        self
    }

    pub fn index_mid(mut self, value: u16, style: Style) -> Self {
        self.indices[1] = Some(CardGlyphIndex { value, style });
        self
    }

    pub fn index_bottom(mut self, value: u16, style: Style) -> Self {
    ...
    pub fn push_to_rows(&self, rows: &mut [Vec<Span<'static>>; 3]) {
...
impl MiniCardGlyph {
    ...
    pub fn bracket_styles(mut self, left: Style, right: Style) -> Self {
    ...
    pub fn push_to(&self, target: &mut Vec<Span<'static>>) {
...
pub fn append_gap(rows: &mut [Vec<Span<'static>>; 3], gap: &'static str) {
    for row in rows {
        row.push(Span::raw(gap));
    }
}

pub fn join_lines_horizontal(
    left: &[Line<'static>],
...
impl CliUi {
    pub fn new(view_mode: CliViewMode) -> io::Result<Self> {
    ...
    pub fn view_mode(&self) -> CliViewMode {
        self.view_mode
    }

    pub fn show_model(&mut self, model: &GameProjection, message: String) -> io::Result<()> {
    ...
    pub fn show_observer_model(
        ...
        self.observer_summary = Some(game_projection_summary(model));
    ...
    pub fn set_active_player(&mut self, player_id: Option<PlayerId>) {
    ...
    pub fn prompt(&mut self, model: &GameProjection, prompt: &str) -> io::Result<String> {
    ...
    pub fn poll_control_input(&mut self) -> io::Result<Option<ControlInput>> {
    ...
    pub fn select_intersection_where(
        &mut self,
        model: &GameProjection,
        prompt: &str,
    ...
    pub fn select_build_with_preview(
        &mut self,
        model: &GameProjection,
        builds: Vec<Build>,
        prompt: &str,
    ...
    pub fn select_trade_response_action(
        &mut self,
        model: &GameProjection,
    ...
    pub fn select_trade_owner_action(
        &mut self,
        model: &GameProjection,
        session_id: TradeSessionId,
    ...
    pub fn select_player_trade_offer(
        &mut self,
        model: &GameProjection,
...
fn trade_offer_is_accepted(session: &TradeSessionProjection, offer_id: TradeOfferId) -> bool {
...
fn trade_response_actions_for_offer(
    private: Option<&PrivatePlayerProjection>,
    offer: &TradeOfferProjection,
) -> &'static [&'static str] {
...
fn player_event_style(player_id: PlayerId) -> Style {
    Style::default().fg(crate::ratatui_adapter::color(
        crate::field::FieldRenderer::player_color(player_id),
    ))
}
...
fn resource_event_style_for(resource: Resource) -> Style {
    Style::default().fg(crate::ratatui_adapter::color(
        crate::field::FieldRenderer::resource_color(resource),
    ))
}

fn render_bank(frame: &mut Frame<'_>, area: Rect, model: &GameProjection) {
...
#[cfg(test)]
mod tests {
    use catan_core::gameplay::game::projection::{PrivatePlayerProjection, TradeOfferProjection};
    use catan_core::gameplay::{
    ...
    fn journal_resource_markers_render_as_colored_mini_cards() {
        ...
        let brick_color = crate::ratatui_adapter::color(
            crate::field::FieldRenderer::resource_color(Resource::Brick),
        );
        assert!(
            line.spans
                .iter()
    ...
    fn trade_response_actions_hide_accept_when_viewer_cannot_pay() {
        let offer = TradeOfferProjection {
            id: TradeOfferId(0),
        ...
        let private = PrivatePlayerProjection {
