...
use crate::field::SelectionStatus;
use catan_core::gameplay::game::projection::{GameProjection, LegalDecisionOptions};
...
pub fn selection_status(is_available: bool) -> SelectionStatus {
    if is_available {
...
pub fn move_hex_by_key(current: Hex, key: KeyCode, board_hexes: &BTreeSet<Hex>) -> Hex {
    let next = match key {
...
fn board_hexes(model: &GameProjection) -> Vec<Hex> {
    (0..model.public.board.tiles.len())
        .map(HexIndex::spiral_to_hex)
        .collect()
}

pub fn board_hex_set(model: &GameProjection) -> BTreeSet<Hex> {
    board_hexes(model).into_iter().collect()
}

pub fn legal_initial_settlements(legal: &LegalDecisionOptions) -> BTreeSet<Intersection> {
    legal
        .initial_placements
        .iter()
...
pub fn initial_roads_for_settlement(
...
pub fn legal_builds_for_mode(legal: &LegalDecisionOptions, mode: PartialBuildMode) -> Vec<Build> {
    match mode {
...
pub fn knight_hexes(legal: &LegalDecisionOptions) -> BTreeSet<Hex> {
    legal
        .dev_card_usages
        .iter()
...
pub fn knight_rob_targets(legal: &LegalDecisionOptions, hex: Hex) -> Vec<PlayerId> {
    legal
        .dev_card_usages
        .iter()
        .filter_map(|usage| match usage {
...
pub fn roadbuild_first_options(legal: &LegalDecisionOptions) -> Vec<Build> {
    legal
        .dev_card_usages
        .iter()
        .filter_map(|usage| match usage {
...
pub fn roadbuild_second_options(legal: &LegalDecisionOptions, first: BoardPath) -> Vec<Build> {
    legal
        .dev_card_usages
        .iter()
        .filter_map(|usage| match usage {
...
pub fn ordered_bank_trades_for_menu(legal: &LegalDecisionOptions) -> Vec<BankTrade> {
...
mod tests {
    ...
    fn bank_trade_menu_orders_ports_before_bank_trades() {
        let legal = catan_core::gameplay::game::projection::LegalDecisionOptions {
            bank_trades: vec![
                BankTrade {
                    give: Resource::Brick,
                    take: Resource::Wood,
