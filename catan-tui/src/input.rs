...
use crate::cli_command::{CliCommand, DevCardCommand, ParseCommandError, parse_cli_command};
use crate::field::FieldPreview;
use catan_core::gameplay::game::projection::{DecisionRequestEnvelope, GameProjection};
use catan_core::{
    constants,
...
pub fn read_trade_response_action(
    ui: &mut CliUi,
...
pub fn read_trade_owner_action(
    ui: &mut CliUi,
    model: &GameProjection,
    session_id: TradeSessionId,
...
fn build_unavailable_reason(model: &GameProjection, kind: PartialBuildMode) -> String {
...
fn player_piece_count(model: &GameProjection, actor: PlayerId, kind: PartialBuildMode) -> usize {
...
fn set_invalid_use(ui: &mut CliUi, cause: String) -> io::Result<()> {
    ...
    log::warn!(target: "catan_tui::input", "{message}");
    ui.set_message(message)
}
...
pub fn read_initial_settlement(
    ui: &mut CliUi,
    envelope: &DecisionRequestEnvelope,
    prompt: &str,
...
fn read_player_id(ui: &mut CliUi, model: &GameProjection, prompt: &str) -> io::Result<PlayerId> {
